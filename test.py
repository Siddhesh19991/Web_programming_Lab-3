...
class FlaskAppTests(unittest.TestCase):
        ...
        for index, email in enumerate(emails):
            ...
            try:
                ...
                if index == 0:
                    ...
                    self.assertFalse(
                        is_positive, "It looks like the sign up was successful with duplicate email.")
                    print(f"Test_01_sign_up duplicate email passed: âœ…")
                else:
                    self.assertTrue(
                        is_positive, "It looks like the sign up was not successful for 2nd user.")
    ...
    def test_02_sign_up_invalidate_data(self):
        ...
        invalid_emails = [f'{invalid_email}@example',
                          f'{invalid_email}@.com', f'{invalid_email}@exa mple.com']
        ...
        for email in invalid_emails:
            ...
            try:
                ...
                self.assertFalse(
                    is_positive, f"It looks like the sign up was successful with invalid email {email}.")
                print(f"test_02_sign_up_invalidate_data passed for invalid email {
                      email}: âœ…")
            except requests.exceptions.HTTPError as e:
                logging.warning(f"HTTPError occurred: {e}")
                print(f"test_02_sign_up_invalidate_data crashed for invalid email {
                      email}: âŒ")
            except requests.exceptions.RequestException as e:
                self.fail(f"RequestException occurred: {e}")
            except AssertionError as ae:
                print(f"test_02_sign_up_invalidate_data failed for invalid email {
                      email}: âŒ {ae}")

        # List of fields to exclude in each iteration
        fields_to_exclude = ['email', 'password', 'firstname',
        ...
        for field in fields_to_exclude:
            ...
            try:
                ...
                self.assertFalse(
                    is_positive, f"It looks like the sign up was successful with missing {field}.")
                print(f"test_02_sign_up_invalidate_data passed for missing {
                      field}: âœ…")
            except requests.exceptions.HTTPError as e:
                logging.warning(f"HTTPError occurred: {e}")
                print(f"test_02_sign_up_invalidate_data crashed for missing {
                      field}: âŒ")
            except requests.exceptions.RequestException as e:
                self.fail(f"RequestException occurred: {e}")
            except AssertionError as ae:
                # Catching AssertionError and printing a custom failure message
                print(f"test_02_sign_up_invalidate_data failed for missing {
                      field}: âŒ {ae}")
        ...
        for password in passwords:
            ...
            try:
                ...
                if password == passwords[0]:
                    ...
                    self.assertFalse(
                        is_positive, "It looks like the sign in was successful with incorrect password.")
                    print(f"test_03_sign_in incorrect password passed: âœ…")
                else:
                    self.assertFalse(
                        is_positive, "It looks like the sign in was successful with missing password.")
    ...
    def test_04_change_password(self):
        ...
        for i in range(len(combinations_dict_hardcoded)):
            try:
                headers = {
                    'Authorization': combinations_dict_hardcoded[i]["tokens"]}
                ...
                if i == 0:
                    ...
                    self.assertTrue(
                        is_positive, "It looks like the change password was not successful.")
    ...
    def test_05_get_user_data_by_token(self):
        ...
        for token in tokens:
            try:
                ...
                if token == tokens[0]:
                    self.assertTrue(
                        is_positive, "It looks like the get user data by token was not successful.")
    ...
    def test_06_get_user_data_by_email(self):
        ...
        for i in range(len(combination_dict)):
            try:
                ...
                url = f'{
                    self.base_url}/get_user_data_by_email/{combination_dict[i]["email"]}'
                response = requests.get(url, headers=headers)
                response.raise_for_status()
                result = response.json()
                ...
                if i == 0:
                    self.assertTrue(
                        is_positive, "It looks like the get user data by email was not successful.")
                    ...
                    self.assertTrue(
                        is_positive, "It looks like the get 2nd user data by email was not successful.")
    ...
    def test_07_post_message(self):
        ...
        for i in range(len(combinations_dict_hardcoded)):
            try:
                headers = {
                    'Authorization': combinations_dict_hardcoded[i]["token"]}
                ...
                if i == 0:
                    ...
                    self.assertTrue(
                        is_positive, "It looks like the post message was not successful on 2nd user wall.")
                    print(f"test_07_post_message 2nd user post on wall passed: âœ…")
                else:
                    self.assertTrue(
                        is_positive, "It looks like the post message was not successful on main user wall.")
    ...
    def test_08_get_user_messages_by_token(self):
        ...
        for token in tokens:
            try:
                ...
                if token == tokens[0]:
                    self.assertTrue(
                        is_positive, "It looks like the get user messages by token was not successful.")
                    print(
    ...
    def test_09_get_user_messages_by_email(self):
        ...
        for i in range(len(combination_dict)):
            try:
                headers = {'Authorization': combination_dict[i]["token"]}
                url = f'{
                    self.base_url}/get_user_messages_by_email/{combination_dict[i]["email"]}'
                response = requests.get(url, headers=headers)
                response.raise_for_status()
                result = response.json()
                ...
                if i == 0:
                    self.assertTrue(
                        is_positive, "It looks like the get user messages by email was not successful.")
                    print(
                    ...
                    self.assertTrue(
                        is_positive, "It looks like the get 2nd user messages by email was not successful.")
        ...
        for token in tokens:
            try:
                ...
                if token == tokens[0]:
                    ...
                    self.assertFalse(
                        is_positive, "It looks like the sign out was successful with invalid token.")
                    print(f"test_10_sign_out incorrect token passed: âœ…")
                else:
                    self.assertFalse(
                        is_positive, "It looks like the sign out was successful with missing token.")
...
def set_up_base_url():
    ...
    address = input(
        f'Enter address (default {default_address}): ') or default_address
